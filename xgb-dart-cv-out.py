...
def runXGB(train_X, train_y, test_X, test_y=None, feature_names=None,      
           ...
           max_depth = 6,cv_dict = None,verbose_eval=True,sb=0.7,cb=0.7):
    param = {}
...
for md in md_list:
    ...
    for dev_index, val_index in KF: 
        ...
        preds,model = runXGB(dev_X, dev_y, val_X, val_y,feature_names=features,\
               early_stop = 64,num_rounds=4500,eta = 0.1,max_depth=md,cv_dict = result_dict,verbose_eval=100)
    ...
    if min_loss>np.mean(cv_scores):
        min_loss = np.mean(cv_scores)
        min_md = md
...
for sb in sb_list:
    for cb in cb_list:
        ...
        for dev_index, val_index in KF: 
            ...
            preds,model = runXGB(dev_X, dev_y, val_X, val_y,feature_names=features,\
                   early_stop = None,num_rounds=64,eta = 0.1,sb=sb,cb=cb,max_depth=md,cv_dict = result_dict,verbose_eval=100)
...
print 'the best params (md,sb,cb) are:'
print (min_md,min_sb,min_cb)
