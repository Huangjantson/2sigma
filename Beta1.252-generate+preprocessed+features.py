...
def addAvgDiff(train_df,test_df,nn=20):
    #adding features
    """
    ['diff_price','diff_price_per_bed','diff_price_per_bath','diff_price_per_room']
    """
    ...
    for i in train_df.index:
        if ~np.isnan(train_df.ix[i,'latitude']) and ~np.isnan(train_df.ix[i,'longitude']):
            ...
            train_df.ix[i,'diff_price'] = train_df.ix[i,'price'] - temp.mean()
    ...
    for i in test_df.index:
        if ~np.isnan(test_df.ix[i,'latitude']) and ~np.isnan(test_df.ix[i,'longitude']):
            ...
            temp = train_test.iloc[idx.reshape(idx.size).tolist(),:].ix[:,'price']
            temp = temp[temp!=np.inf]
            test_df.ix[i,'diff_price'] = test_df.ix[i,'price'] - temp.mean()
...
for f in categorical:
    ...
    if train_set[f].dtype=='object':
        ...
        lbl.fit(list(train_set[f])+list(test_set[f]))
...
"""
train_set[features_to_use+['interest_level']].to_csv('preprocessed-train-beta1.252.csv')
test_set[features_to_use].to_csv('preprocessed-test-beta1.252.csv')
