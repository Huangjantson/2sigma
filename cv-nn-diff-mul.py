...
def addAvgDiff(train_df,test_df,nn=20):
    ...
    """
    features_to_edit = ['price','price_per_bed','price_per_bath','price_per_room']
    features_to_append = []
    for i in range(len(features_to_edit):
        features_to_append[i] = 'diff_'+features_to_edit[i]+'_'+str(nn)
        train_df[features_to_append[i]] = np.nan
        test_df[features_to_append[i]] = np.nan
    ...
    for i in train_df.index:
        if ~np.isnan(train_df.ix[i,'latitude']) and ~np.isnan(train_df.ix[i,'longitude']):
            ...
            for j in range(len(features_to_edit):
               temp = train_test.iloc[idx.reshape(idx.size).tolist(),:].ix[:,features_to_edit[j]]
               temp = temp[temp!=np.inf]
               train_df.ix[i,features_to_append[j]] = train_df.ix[i,features_to_edit[j]] - temp.mean()
    ...
    for i in test_df.index:
        if ~np.isnan(test_df.ix[i,'latitude']) and ~np.isnan(test_df.ix[i,'longitude']):
            ...
            for j in range(len(features_to_edit):
               temp = train_test.iloc[idx.reshape(idx.size).tolist(),:].ix[:,features_to_edit[j]]
               temp = temp[temp!=np.inf]
               test_df.ix[i,features_to_append[j]] = test_df.ix[i,features_to_edit[j]] - temp.mean()
