...
train_df, test_df =manager_skill_eval(train_df,test_df,\
unrank_threshold = mini_ranking)
features_to_use.append('manager_skill')

#fill substitute the small size values by their mean
for f in categorical:
    train_df,test_df  = singleValueConvert(train_df,test_df,f,mini_ranking)

    if train_df[f].dtype=='object':
        #print(f)
        lbl = preprocessing.LabelEncoder()
        lbl.fit(list(train_df[f])+list(train_df[f]))
...
preds, model = runXGB(train_X, train_y, test_X_m, \
feature_names=features_to_use,num_rounds = 6000, eta = 0.01,max_depth = 4)

out_df = pd.DataFrame(preds)
out_df.columns = ["high", "medium", "low"]
out_df["listing_id"] = test_df.listing_id.values
out_df.to_csv("xgb_beta1point251a-pure-out.csv", index=False)
