...
#===================================================================
        ...
        #construct the validation set for the ith turn
        validationSet[i] = {'dev_X':dev_X,'val_X':val_X,'dev_y':dev_y,'val_y':val_y}
        i+=1
        
result_file = 'md.log'

#xgb_eta = [0.1,0.03,0.01,0.003,0.001]
xgb_md =  [3,4,5,6,7]
eta = 0.1
"""
find the best eta
"""
early = lambda x: int(np.log(x)**2*5)

for md in xgb_md:
    print "testing max_depth is :"+str(md)
    cv_scores = []
    for i in validationSet:
        dev_X = validationSet[i]['dev_X']
        val_X = validationSet[i]['val_X']
        dev_y = validationSet[i]['dev_y']
        val_y = validationSet[i]['val_y']
        """
        ...
        feature_names=features_to_use,early_stop=early(eta),
        num_rounds = 20000, eta = eta,max_depth = md)
    
    cv_scores.append(log_loss(val_y, preds))

    print 'The validaiton for max_depth = '+str(md)+'is ended,results are below'
    print cv_scores
    ...
    try:
        ...
        f.write('the cv-score for max depth with value '\
        +str(md)+' is: '\
        +str(cv_scores)+'\n')
