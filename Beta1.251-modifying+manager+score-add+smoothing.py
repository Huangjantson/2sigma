...
def manager_skill_eval(train_df,test_df,k,g=1,f=1,update_df =None):
...
for g in g_list:
    ...
    #====================================================================        
            ...
            for train,test in skf:
                manager_skill_eval(dev_set.iloc[train,:],dev_set.iloc[test,:],k=5,g=1,update_df = dev_set)
                
            #val set adding manager skill
            manager_skill_eval(dev_set,val_set,k=5,f=1)
    ...
    try:
        ...
        f.write('the cv-score for smoothing parameter g with value '\
        +str(g)+' is: '\
        +str(cv_scores)+'\n')
