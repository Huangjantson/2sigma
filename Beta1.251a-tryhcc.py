...
#instead use a simpler version for hcc-encoding which is taking the mean

def hcc_scoring(train_df,test_df,feature,labelValue,update_df =None,unrank_threshold = 15,randomize = 0.1):    
    ...
    grouped = tempTrain.groupby(feature)[labelValue].agg({'size':'size','mean':'mean'})
    
    grouped[new_feature] = grouped['mean']
    
    #get the average score for the unrank features and reset them to this average
    unrankedMean = grouped.ix[grouped['size']<unrank_threshold,new_feature].mean()
    grouped.ix[grouped['size']<unrank_threshold,new_feature] = unrankedMean
...
for f , s in KF:
    ...
    hcc_scoring(train_df.iloc[f],train_df.iloc[s],'manager_id','high',update_df =train_df,unrank_threshold = 15,randomize = 0.1)
    hcc_scoring(train_df.iloc[f],train_df.iloc[s],'manager_id','medium',update_df =train_df,unrank_threshold = 15,randomize = 0.1)
    hcc_scoring(train_df.iloc[f],train_df.iloc[s],'building_id','high',update_df =train_df,unrank_threshold = 15,randomize = 0.1)
    hcc_scoring(train_df.iloc[f],train_df.iloc[s],'building_id','medium',update_df =train_df,unrank_threshold = 15,randomize = 0.1)
